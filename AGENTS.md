...
- **2026-04-18 (cleanup):** Per user request, removed emulation-specific hacks now that we've committed to Linux deployment. Dropped `CPU_MHZ=3000` env from Dockerfile. Collapsed the long "ARM Mac limitation" section in USAGE.md down to two sentences — the full investigation remains here in AGENTS.md for historical reference. Kept `--platform=linux/amd64` everywhere: that's not a hack, it's an accurate declaration of the image's required arch.
