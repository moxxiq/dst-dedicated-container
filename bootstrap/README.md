...
and you just want to retry specific steps — run them **as the `dst` user**.
`podman` is rootless and all state (images, containers, volumes) lives
under the `dst` user's XDG_RUNTIME_DIR, so running these as `root` won't
help:
...
Simplest recovery: just re-run the full bootstrap script — it's idempotent.
...
| `podman-compose` crashes with `TypeError: join() argument must be str, ... not 'dict'` | Your admin or monitoring `docker-compose.yml` is using the Compose-Spec long-form `env_file: - path: ... required: true`. `podman-compose` 1.0.6 (Ubuntu 24.04's apt version) mishandles it — use short-form `env_file: - ../.env` instead. |
| `podman compose: command not found` (with a space) | You're on an old copy of the bootstrap. Re-pull `vultr-bootstrap.sh` from master, or `sudo apt install podman-compose`. |
| Beszel hub shows agent as "Down" | You haven't pasted the hub's per-agent SSH key yet. See `monitoring/README.md` step 3. |
| `libcurl-gnutls.so.4: cannot open shared object file` in DST logs | Container missing the `libcurl3-gnutls` package. Pull master and rebuild: `cd ~/steamCMD && git pull && podman build -t local/steamcmd:latest .` |
| `pasta: executable file not found` during `podman build` on Ubuntu 24.04 | Bootstrap's apt list is missing `passt`. Pull master and re-run, or `sudo apt install passt` and retry. |
| Containers stay in `Created` state after host reboot | Linger is on but the per-user `podman-restart.service` wasn't enabled. Bootstrap from master enables it; for an existing host: `systemctl --user --machine=dst@.host enable --now podman-restart.service`. |
| `podman ps` hangs forever | Stuck `podman ps` from a prior run holds the sqlite db lock. `pkill -9 -f 'podman ps'` then retry. Worst case `sudo reboot`. |
| Parked clusters from a Mac-made zip show as invalid (Master/Caves dashes) | The zip embedded `__MACOSX/` metadata which broke the flatten heuristic. Pull master + rebuild admin (`podman build -t local/dst-admin:latest admin && podman restart dst-admin`); old parked dirs need manual `rm -rf parked/<name>/__MACOSX && mv parked/<name>/<inner>/* parked/<name>/` cleanup. |
